 ...
 * buffers
 */
static size_t compare_buffers(const void *b1, const void *b2, const size_t len)
{
                    ...
                    const size_t eeeprom_entry_size, const uint16_t vfo_version)
{
    ...
    s->slicer_conf.struct_size = sizeof(struct vfo);
    s->slicer_conf.slice_size = eeeprom_entry_size;
...
int vfo_save(struct vfo_storage *s, const channel_t *vfo)
{
    if (s->full_write_needed) {
        ...
        while (n != sizeof(struct vfo)) {
            ...
            n = compare_buffers(&(s->saved_vfo), &buffer, sizeof(struct vfo));
        }
    }
