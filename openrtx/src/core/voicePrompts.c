...
static const uint32_t VOICE_PROMPTS_DATA_AMOUNT =
...
static void fetchCodec2Data(uint8_t *data, const size_t offset)
{
    ...
    size_t start = sizeof(vpHeader_t) + sizeof(tableOfContents)
                 + CODEC2_HEADER_SIZE;
...
static inline bool checkVpHeader(const vpHeader_t *header)
{
    return ((header->magic == VOICE_PROMPTS_DATA_MAGIC)
            ...
            && (header->numPrompts == VOICE_PROMPTS_DATA_AMOUNT));
}
                                             ...
                                             enum voicePrompt *vp)
{
    *vp = PROMPT_SILENCE;
    ...
    bool announceCommonSymbols = (flags & vpAnnounceCommonSymbols) ? true :
                                                                     false;
    bool announceLessCommonSymbols = (flags & vpAnnounceLessCommonSymbols) ?
                                         true :
                                         false;
...
void vp_tick()
{
    ...
    while (vpCurrentSequence.pos < vpCurrentSequence.length) {
        ...
        vpCurrentSequence.c2DataLength = 0; // flag that we need more data.
        vpCurrentSequence.c2DataIndex = 0;
    }
