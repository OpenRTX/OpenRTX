...
/* Symbol fonts (GFXfont format, generated by generate_symbols.py) */
...
/**
 * Symbol fonts in GFXfont format, indexed by symbolSize_t.
 */
static const GFXfont symbol_fonts[] = {
    Symbols5pt7b,   // SYMBOLS_SIZE_5PT
    Symbols6pt7b,   // SYMBOLS_SIZE_6PT
    Symbols8pt7b    // SYMBOLS_SIZE_8PT
};

/**
 * Decode one UTF-8 character and return its Unicode codepoint.
 * Advances *bytes_consumed by the number of bytes read.
 * Returns 0xFFFF on invalid sequences.
 */
static uint16_t decode_utf8(const unsigned char *str, size_t *bytes_consumed)
{
    ...
    *bytes_consumed = 1;
    ...
    else if ((c & 0xF0) == 0xE0 && (str[1] & 0xC0) == 0x80 &&
             (str[2] & 0xC0) == 0x80)
    {
        codepoint = ((c & 0x0F) << 12) | ((str[1] & 0x3F) << 6) |
                    (str[2] & 0x3F);
        *bytes_consumed = 3;
    }
    else
    {
        return 0xFFFF;  // unsupported or invalid
    }
...
/**
 * Binary search for a glyph by codepoint in a UniFont.
 * Returns pointer to the glyph, or NULL if not found.
 */
static const UniGlyph* uni_find_glyph(const UniFont *f, uint16_t codepoint)
{
    int lo = 0;
    int hi = (int)f->numGlyphs - 1;

    while (lo <= hi)
    {
        int mid = (lo + hi) / 2;
        uint16_t midCp = f->glyphs[mid].codepoint;
...
/**
 * Compute the pixel size of the first text line using UniFont
 * @param f: pointer to UniFont used as the source of glyphs
 * @param text: the input text (UTF-8)
 * @param length: the length of the input text in bytes
 */
static inline uint16_t get_line_size(const UniFont *f, const char *text,
                                     uint16_t length)
{
    ...
    for(unsigned i = 0; i < length && text[i] != '\n' && text[i] != '\r'; )
    {
        ...
        uint16_t cp = decode_utf8((const unsigned char*)&text[i], &bytes_consumed);
        i += bytes_consumed;

        if (cp == 0xFFFF)
            continue;

        const UniGlyph *glyph = uni_find_glyph(f, cp);
        if (glyph == NULL)
            continue;
...
{
    const UniFont *f = &text_fonts[size];
    const UniGlyph *glyph = uni_find_glyph(f, '|');
    if (glyph == NULL)
        return f->yAdvance;  // fallback
    return glyph->height;
}
                        ...
                        color_t color, const char *buf)
{
    ...
    /* For each char in the string (UTF-8) */
    for(unsigned i = 0; i < len; )
    {
        // Decode UTF-8 to codepoint
        size_t bytes_consumed = 0;
        uint16_t cp = decode_utf8((const unsigned char*)&buf[i], &bytes_consumed);
        i += bytes_consumed;
        ...
        // Look up glyph by codepoint
        const UniGlyph *glyph = uni_find_glyph(f, cp);
        if (glyph == NULL)
            continue;  // glyph not in font, skip
        ...
        {
            // Compute size of the remaining text in pixels
            line_size = get_line_size(f, &buf[i], len - i);
            start.x = reset_x = get_reset_x(alignment, line_size, start.x);
            start.y += f->yAdvance;
        }
                       ...
                       color_t color, symbol_t symbol)
{
    ...
    uint16_t sym_width = glyph.xAdvance;
    uint16_t reset_x = get_reset_x(alignment, sym_width, start.x);
    start.x = reset_x;
