...
const char* menu_items[] = {"Zone",     "Channels", "Contacts",
#ifdef HAS_GPS
                            "GPS",
#endif
                            "Settings", "Info",     "About"};
...
const char* display_items[] = {"Brightness",
...
const char* symbols_ITU_T_E161_callsign[] = {"0",    "1",     "ABC2", "DEF3",
...
int _ui_fsm_loadChannel(uint16_t zone_index, bool* sync_rtx)
{
    ...
    {
        // Calculate zone size
        const uint8_t zone_size =
            sizeof(state.zone.member) / sizeof(state.zone.member[0]);
...
void _ui_changeBrightness(int variation)
{
    if (variation >= 0)
        ...
        state.settings.brightness = (state.settings.brightness < -variation)
                                        ? 0
                                        : state.settings.brightness + variation;
                         ...
                         bool callsign)
{
    ...
    if (ui_state.last_keypress != 0)
    {
        ...
        if ((ui_state.input_number == num_key) &&
            ((now - ui_state.last_keypress) < kbd_long_interval))
        {
        ...
        switch (state.ui_screen)
        {
            ...
            case MAIN_VFO:
                if (msg.keys & KEY_UP || msg.keys & KNOB_RIGHT)
                {
                    ...
                    if (_ui_freq_check_limits(state.channel.rx_frequency +
                                              12500) &&
                ...
                else if (input_isNumberPressed(msg))
                {
                    ...
                    ui_state.new_tx_frequency = 0;
                    // Save pressed number to calculare frequency and show in
                    // GUI
                    ui_state.input_number = input_getPressedNumber(msg);
                ...
                if (state.channel.mode == M17)
                {
                    ...
                    if (ui_state.edit_mode)
                    {
                        if (msg.keys & KEY_ENTER)
                        {
                            ...
                            strncpy(state.m17_data.dst_addr,
            ...
            case MENU_CONTACTS:
                ...
                else if (msg.keys & KEY_DOWN || msg.keys & KNOB_RIGHT)
                {
                    ...
                    else if (state.ui_screen == MENU_CHANNEL)
                    {
                        channel_t channel;
                        // menu_selected is 0-based while channels are 1-based
                        if (nvm_readChannelData(
                                &channel, ui_state.menu_selected + 2) != -1)
                            ui_state.menu_selected += 1;
                    }
                    else if (state.ui_screen == MENU_CONTACTS)
                    {
                        contact_t contact;
                        // menu_selected is 0-based while channels are 1-based
                        if (nvm_readContactData(
                                &contact, ui_state.menu_selected + 2) != -1)
                            ui_state.menu_selected += 1;
                    }
                }
                else if (msg.keys & KEY_ENTER)
                {
                    if (state.ui_screen == MENU_ZONE)
                    {
                        ...
                        else
                        {
                            ...
                            result             = nvm_readZoneData(&newzone,
                    ...
                    if (state.ui_screen == MENU_CHANNEL)
                    {
                        ...
                        if (ui_state.last_main_state == MAIN_VFO)
                            state.vfo_channel = state.channel;
                        _ui_fsm_loadChannel(ui_state.menu_selected + 1,
                                            sync_rtx);
                        // Switch to MEM screen
                        state.ui_screen = MAIN_MEM;
                    }
            ...
            case SETTINGS_TIMEDATE_SET:
                if (msg.keys & KEY_ENTER)
                {
                    ...
                    // time
                    curTime_t utc_time =
                        state_getUTCTime(ui_state.new_timedate);
            ...
            case SETTINGS_M17:
                if (ui_state.edit_mode)
                {
                    if (msg.keys & KEY_ENTER)
                    {
                        ...
                        strncpy(state.m17_data.callsign, ui_state.new_callsign,
                                10);
                        ui_state.edit_mode = false;
                        *sync_rtx          = true;
                    }
