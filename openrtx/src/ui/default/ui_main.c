...
void _ui_drawMainTop(ui_state_t *ui_state)
{
    ...
    gfx_print(layout.top_pos, layout.top_font, TEXT_ALIGN_RIGHT, color_white,
              "%u.%uV", last_state.v_bat / 1000,
              (last_state.v_bat % 1000) / 100);
#else
    if (last_state.settings.showBatteryIcon) {
        ...
        point_t bat_pos = { CONFIG_SCREEN_WIDTH - bat_width
        ...
        gfx_print(bat_pos, FONT_SIZE_6PT, TEXT_ALIGN_RIGHT, color_white, "%d%%",
                  last_state.charge);
    }
#endif
    point_t left_pos = layout.top_pos;
    if (ui_state->input_locked == true) {
        left_pos = gfx_drawSymbol(left_pos, layout.top_symbol_size,
                                  TEXT_ALIGN_LEFT, color_white, SYMBOL_LOCK);
        left_pos.x += layout.horizontal_pad;
    }
    if (messages_count_unread() > 0)
        gfx_drawSymbol(left_pos, layout.top_symbol_size, TEXT_ALIGN_LEFT,
