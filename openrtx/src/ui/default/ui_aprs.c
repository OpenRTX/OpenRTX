 ...
 * @param address The aprsAddress to be converted to text
 */
void aprsAddrStr(char *outStr, struct aprsAddress address)
{
    if (address.ssid)
...
{
    struct aprsPacket *pkt = ui_state->pkt;
    ...
    if (msg.keys & KEY_UP || msg.keys & KNOB_LEFT) {
        ...
        state->aprsStoredPkts = aprsPktList_delete(state->aprsStoredPkts, pkt);
    }
}

/**
 * @brief Shows a view of an APRS packet on the screen
 *
 * @param ui_state The current state of the user interface
 */
void ui_showAprsPkt(ui_state_t *ui_state)
{
    char addressStr[10] = "";
    struct aprsPacket *pkt = ui_state->pkt;
    ...
    char *devId = pkt->addresses[0].addr;
    // the second address is the src, make an address string from it
    aprsAddrStr(addressStr, pkt->addresses[1]);
    ...
    char digipeaters[32] = "";
    // all the following addresses are digipeater addresses
    for (uint8_t i = 2; i < pkt->addressesLen; i++) {
        aprsAddrStr(addressStr, pkt->addresses[i]);
