...
IMA-ADPCM (4-bit, integer decode, ~4 KB/s) on the host:
    ...
    .vpc prompt bytes --(c2dec 3200)--> s16 PCM 8 kHz --(IMA-ADPCM)--> nibbles

Modes:
  clip   <vpc> <c2dec> <idx> <out.h>   -- one prompt -> C header (Stage A test)
  pack   <vpc> <c2dec> <out.bin>       -- whole vocabulary -> ADPCM container

IMA-ADPCM here is mono, 8 kHz, no block headers: each clip is a flat nibble
stream decoded from a fixed start state (predictor=0, index=0).  Low nibble of
each byte is the earlier sample.  Matches the on-target decoder in
lib/codec/adpcm_ima.h, driven by core/voicePrompts_adpcm.c.
"""
import struct
import subprocess
import sys
...
def c2_to_pcm(c2dec, c2bytes):
    """Run c2dec 3200 over a codec2 bitstream -> list[int16] PCM."""
...
def prompt_bytes(toc, data, idx):
    start = toc[idx]
    length = (toc[idx + 1] - toc[idx]) // 8 * 8   # whole 3200 frames only
...
def cmd_clip(vpc, c2dec, idx, out_h):
    ...
    pcm = c2_to_pcm(c2dec, prompt_bytes(toc, data, idx))
...
def cmd_pack(vpc, c2dec, out_bin, include=None):
    ...
    for idx in range(350):
        ...
        if not b:
            clips.append(b"")
            continue
        clips.append(ima_encode(c2_to_pcm(c2dec, b)))
    # container: magic 'VPA1', count, TOC[count+1] u32 byte offsets, data
    blob = bytearray()
