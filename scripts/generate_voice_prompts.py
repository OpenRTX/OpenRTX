...
2. Parses EnglishStrings.h to extract stringsTable_t field names and text values
...
class VoicePromptGenerator:
    ...
    def __init__(self, project_root: Path, overrides_file: Path = None,
        ...
        self.ui_strings_header = ui_strings_header or openrtx_dir / "include/ui/EnglishStrings.h"
    ...
    def parse_strings_table(self) -> List[Tuple[str, str]]:
        """
        Parse EnglishStrings.h to extract stringsTable_t field names and text
        values from the englishStrings initializer.
...
Examples:
  ...
  %(prog)s --check --tempo 1.3
    ...
    parser.add_argument(
        '--ui-strings-header',
        type=Path,
        help='Path to EnglishStrings.h (default: <project-root>/openrtx/include/ui/EnglishStrings.h)'
    )
