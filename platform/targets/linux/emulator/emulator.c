...
keyboard_t keyname2keyboard(char* name)
{
    /*  The line noise at the end of this comment is a vim macro for taking the
       keyboard.h interface and putting it into the format further below You can
       load it into vim register k with "kyy and run the macro with @k (and then
       you can repeat a macro register application with @@ ) (substitute k with
       any register you like) Once you've got all the names quoted, you can J
       them all together into a nice block.
    ...
    */
    ...
    for (int i = 0; i < numnames; i++)
    {
        ...
        {
            /*printf("MATCH with %s\n", names[i]);*/
            //+4 to skip the KEY_ on all the names, non +4 to allow for
            //KNOB_LEFT. This also means you can write KEY_LEFT as "KEY_LEFT",
            // or "LEFT" and KNOB_LEFT as "KNOB_LEFT" or "_LEFT"

            // so if name == "2", this whole function will return equivalent to
            // KEY_2 cpp define and if name=="LEFT", then you get equivalent to
            // KEY_LEFT cpp define and if name=="_LEFT", then you get equivalent
            // to KNOB_LEFT cpp define and if name=="KNOB_LEFT", then you get
            // equivalent to KNOB_LEFT cpp define and if name=="KEY_2", then you
            // get equivalent to KEY_2 cpp define of course
            return (1 << i);
    ...
    for (int i = 0; i < _argc; i++)
    {
        if (_argv[i] != NULL)
        {
            ...
            if (press == last)
            {
                ...
                // we only need to do this if we have two identical keys back to
                // back, because keyboard_t will have a zero for this key's flag
                // on other keys, which gives us the release we need
                shellkeyq_put(0);
            }
            shellkeyq_put(press);
            last = press;
...
// need another function to press them in sequence by loading up a queue for
// keypress_from_shell to pull from
...
/*
    ...
    //system("ffmpeg -f x11grab -show_region 1 -region_border 10 -window_id
   0x2600016 -i :0.0 out.mkv");
...
_climenu_option _options[] = {
    /* name/shortcut   description            var reference, if available
       method to call */
    ...
    {"screenshot",
     ...
     "given",
     NULL, screenshot},
    /*{"record_start", "[screen.mkv] Automatically save a video of the remaining
       session (or until record_stop is called)",*/
    /*NULL,   record_start },*/
...
_climenu_option* findMenuOption(char* tok)
{
    for (int i = 0; i < num_options; i++)
    {
        ...
        {
            // strncmp like this allows for typing shortcuts like just "r"
            // instead of the full "rssi" priority for conflicts (like if there's
            // "s" which could mean
    ...
    do
    {
        ...
        switch (ret)
        {
            ...
            case SH_WHAT:
                ...
                ret = SH_CONTINUE;  // i'd rather just fall through, but the
                                    // compiler warns. blech.
                /*printf("\n>");*/
                break;
            case SH_CONTINUE:
                /*printf("\n>");*/
