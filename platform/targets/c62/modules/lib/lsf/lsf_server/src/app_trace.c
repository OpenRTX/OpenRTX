...
write_syslog(uint32_t v1, uint32_t v2)
{
    ...
    xos_log_write((uint32_t) xos_thread_id(), v1, v2);
...
void Trace_printAll(void)
{
    ...
    if (entry == NULL)
        xos_fatal_error(-1, "FAIL: error trying to read syslog\n");
...
static void * logmem;
static int logmem_bytes = XOS_LOG_SIZE(1000);
...
void Trace_dump(void)
{
    ...
    // TODO: 单帧数据区起止/大小非cacheline对齐的影响?
    // TODO: 区别dma和cpu写入, 仅当cpu写入才需要. 用户自己做吧
    xthal_dcache_region_invalidate(logmem, logmem_bytes);
