...
void display_init()
{
    ...
    else
    {
        ...
        // removed RESIZABLE flag so automatic screen recording is a little
        // easier

        renderer = SDL_CreateRenderer(window, -1, 0);
        SDL_RenderSetLogicalSize(renderer, SCREEN_WIDTH, SCREEN_HEIGHT);
        displayTexture = SDL_CreateTexture(renderer, PIXEL_FORMAT,
        ...
        if ((fbSize * 8) < (SCREEN_HEIGHT * SCREEN_WIDTH))
            fbSize +=
                1; /* Compensate for eventual truncation error in division */
        fbSize *= sizeof(uint8_t);
#endif

        /*
         * Grayscale pixel format: framebuffer type is uint8_t where each
         * element controls one pixel
         */
#ifdef PIX_FMT_GRAYSC
