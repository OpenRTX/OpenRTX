...
const uint32_t channelBaseAddrEEPROM =
    0x03780; /**< Base address of channel data         */
const uint32_t channelBaseAddrFlash =
    0x7b1c0; /**< Base address of channel data         */
const uint32_t vfoChannelBaseAddr =
    0x7590; /**< Base address of VFO channel          */
...
int nvm_readVFOChannelData(channel_t* channel)
{
    ...
    AT24Cx_readData(vfoChannelBaseAddr, ((uint8_t*)&chData),
                    sizeof(gdxChannel_t));
...
int nvm_readChannelData(channel_t* channel, uint16_t pos)
{
    ...
    else
    {
        uint32_t channelOffset =
            sizeof(bitmap) + (pos - 1) * sizeof(gdxChannel_t);
        // First channel bank (128 channels) is saved in EEPROM
        if (pos <= 128)
        {
            uint32_t bankAddr =
...
int nvm_readZoneData(zone_t* zone, uint16_t pos)
{
    ...
    gdxZone_t zoneData;
    uint32_t zoneAddr =
        zoneBaseAddr + sizeof(bitmap) + index * sizeof(gdxZone_t);
