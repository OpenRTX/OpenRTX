 ...
 */
static int findActiveBlock( void )
{
    uint16_t block ;
    uint16_t bit ;
    uint16_t crc ;
    ...
    else
    {
        ...
        // Check data validity
        crc    = crc_ccitt( &memory->data[ block ].settings , sizeof( settings_t ) + sizeof( channel_t ) );
...
int nvm_writeSettingsAndVfo( const settings_t* settings , const channel_t* vfo )
{
    ...
    dataBlock.crc = crc_ccitt( &dataBlock.settings , sizeof( settings_t ) + sizeof( channel_t ) );

    // New data is equal to the old one, avoid saving
    if( ( block         != 0       ) &&
        ( dataBlock.crc == prevCrc )    )
    {
        result = 0 ;
//        result = -1 ;
    }
    else
    {
        ...
        result = flash_write( addr , &dataBlock , sizeof( dataBlock_t ) );
        ...
        if( !result )
        {
            ...
            result = flash_write( addr , &flag , sizeof( uint32_t ) );
        }
