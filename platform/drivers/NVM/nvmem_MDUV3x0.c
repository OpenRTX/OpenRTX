...
const uint32_t zoneExtBaseAddr =
    0x31000; /**< Base address of zone extensions                       */
const uint32_t vfoChannelBaseAddr =
    0x2EF00; /**< Base address of VFO channel                         */
...
int nvm_readZoneData(zone_t* zone, uint16_t pos)
{
    ...
    uint32_t zoneExtAddr =
        zoneExtBaseAddr + (pos - 1) * sizeof(mduv3x0ZoneExt_t);
...
int nvm_readContactData(contact_t* contact, uint16_t pos)
{
    ...
    // Note: pos is 1-based to be consistent with channels
    uint32_t contactAddr =
        contactBaseAddr + (pos - 1) * sizeof(mduv3x0Contact_t);
    ...
    // Copy contact DMR ID
    contact->id =
        (contactData.id[0] | contactData.id[1] << 8 | contactData.id[2] << 16);
...
int nvm_readSettings(settings_t* settings)
{
    ...
    W25Qx_sleep();
    if (memcmp(newSettings.valid, default_settings.valid, 6) != 0) return -1;
...
int nvm_writeSettings(settings_t* settings)
{
    ...
    W25Qx_wakeup();
    delayUs(5);
    bool success = W25Qx_writeData(settingsAddr, ((uint8_t*)&settings),
                                   sizeof(settings_t));
    W25Qx_sleep();
    return success ? 0 : -1;
}
