...
uint16_t adcAt32_sample(const struct Adc *adc, const uint32_t channel)
{
    ...
    if (channel < ADC_CHANNEL_10) {
        pAdc->spt2 = (pAdc->spt2 & ~(0x07 << 3 * channel))
                   | (ADC_SAMPLETIME_12_5 << 3 * channel);
    } else if (channel < ADC_CHANNEL_20) {
        pAdc->spt1 = (pAdc->spt1 & ~(0x07 << (3 * (channel - ADC_CHANNEL_10))))
                   | (ADC_SAMPLETIME_12_5 << (3 * (channel - ADC_CHANNEL_10)));
    } else {
        pAdc->spt3 = (pAdc->spt3 & ~(0x07 << (3 * (channel - ADC_CHANNEL_20))))
                   | (ADC_SAMPLETIME_12_5 << (3 * (channel - ADC_CHANNEL_20)));
    }
