...
void HR_Cx000<M>::init()
{
    ...
    writeReg(M::CONFIG, 0x1F,
             0x10);  // Set LocalEMB  DMR Colour code in upper 4 bits -
                     // defaulted to 1, and is updated elsewhere in the code
    writeReg(M::CONFIG, 0x20, 0x00);  // Set LocalAccessPolicy to Impolite
    writeReg(M::CONFIG, 0x21,
             0xA0);  // Set LocalAccessPolicy1 to Polite to Color Code  (unsure
                     // why there are two registers for this)
    ...
    writeReg(M::CONFIG, 0x37, 0x00);  // Set mcu_control_shift to default.
                                      // (codec under HRC-6000 control)
...
void HR_Cx000<M>::uSpi_init()
{
    ...
    SPI0->CTAR[0] = SPI_CTAR_FMSZ(7)      // 8bit frame size
                    ...
                    | SPI_CTAR_PCSSCK(1) | SPI_CTAR_PASC(1) |
                    SPI_CTAR_CSSCK(4) | SPI_CTAR_ASC(4);
}
