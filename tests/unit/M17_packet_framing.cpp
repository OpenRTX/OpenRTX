...
TEST_CASE("PacketFramer: null buffer returns false", "[m17][framing]")
{
    PacketFramer framer;
    REQUIRE(framer.init(nullptr, 10) == false);
}

TEST_CASE("PacketFramer: zero length returns false", "[m17][framing]")
{
    uint8_t data[8] = { 0x05 };
    PacketFramer framer;
    REQUIRE(framer.init(data, 0) == false);
}

// ---------- CRC tests ----------

TEST_CASE("PacketFramer: CRC matches crc_m17 over input data", "[m17][framing]")
{
    ...
    size_t n = frameAll(data, dataLen, frames, 4);
    REQUIRE(n == 1);
...
TEST_CASE("PacketFramer: CRC is big-endian", "[m17][framing]")
{
    uint8_t data[8] = { 0x01, 0x02, 0x03 };
    uint16_t crc = crc_m17(data, 3);

    PacketFrame frames[4];
    frameAll(data, 3, frames, 4);
...
TEST_CASE("PacketFramer: short data fits in one frame", "[m17][framing]")
{
    uint8_t data[8] = { 0x05, 'H', 'i', 0x00 };
    // total = 4 + 2(CRC) = 6 bytes -> 1 frame
    PacketFrame frames[4];
    size_t n = frameAll(data, 4, frames, 4);
    REQUIRE(n == 1);
    REQUIRE(frames[0].isEof() == true);
    REQUIRE(frames[0].getCounter() == 6);
}

TEST_CASE("PacketFramer: data bytes are copied correctly", "[m17][framing]")
{
...
TEST_CASE("PacketFramer: padding after CRC is zero", "[m17][framing]")
{
...
TEST_CASE("PacketFramer: exactly 25-byte total produces one frame",
          "[m17][framing]")
{
    ...
    PacketFrame frames[4];
    size_t n = frameAll(data, 23, frames, 4);
    REQUIRE(n == 1);
    REQUIRE(frames[0].isEof() == true);
...
TEST_CASE("PacketFramer: data spanning two frames", "[m17][framing]")
{
    ...
    PacketFrame frames[4];
    size_t n = frameAll(data, 24, frames, 4);
    REQUIRE(n == 2);
...
TEST_CASE("PacketFramer: three frames with sequential counters",
          "[m17][framing]")
{
    ...
    PacketFrame frames[4];
    size_t n = frameAll(data, 50, frames, 4);
    REQUIRE(n == 3);
...
TEST_CASE("PacketFramer: exactly 50-byte total produces two frames",
          "[m17][framing]")
{
    ...
    PacketFrame frames[4];
    size_t n = frameAll(data, 48, frames, 4);
    REQUIRE(n == 2);
...
TEST_CASE("PacketFramer: nextFrame returns true for last frame",
          "[m17][framing]")
{
    uint8_t data[8] = { 0x05, 'H', 'i', 0x00 };
    PacketFramer framer;
    REQUIRE(framer.init(data, 4) == true);

    PacketFrame frame;
    // Single-frame packet: first call returns true (is last)
    REQUIRE(framer.nextFrame(frame) == true);
}

// ---------- Encoder/decoder round-trip ----------

TEST_CASE("PacketFramer: single frame round-trips through encoder/decoder",
          "[m17][framing]")
{
    uint8_t data[16] = { 0x05, 'O', 'p', 'e', 'n', 'R', 'T', 'X', 0x00 };
    PacketFrame frames[4];
    size_t n = frameAll(data, 9, frames, 4);
    REQUIRE(n == 1);
...
TEST_CASE("PacketFramer: multi-frame round-trips through encoder/decoder",
          "[m17][framing]")
{
    ...
    PacketFrame frames[4];
    size_t n = frameAll(data, 30, frames, 4);
    REQUIRE(n == 2);
...
TEST_CASE("PacketFramer: works with binary data including embedded nulls",
          "[m17][framing]")
{
    // Protocol 0x00 (RAW) with binary content including null bytes
    uint8_t data[8] = { 0x00, 0xFF, 0x00, 0xAB, 0x00, 0xCD };
    PacketFrame frames[4];
    size_t n = frameAll(data, 6, frames, 4);
    REQUIRE(n == 1);
